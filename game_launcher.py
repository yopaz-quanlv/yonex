...
try:
    ...
    raise SystemExit(
        "Pillow is required for game artwork.\n"
        "On macOS with Homebrew, run: brew install pillow"
    ) from error
...
GAME_ROOT = game_root(APP_ROOT)
FCEUX_BASE = Path.home() / ".fceux"
FCEUX_CONFIG = FCEUX_BASE / "fceux.cfg"
FCEUX_INPUT_ROOT = FCEUX_BASE / "input"
GBA_ROOT = Path(os.environ.get("GBA_GAME_DIR", GAME_ROOT / "GBA")).expanduser()
...
BUNDLED_ART = Path(
    os.environ.get("NES_ART_DIR", APP_ROOT / "artwork")
).expanduser()
GAME_METADATA = {
...
class GameLauncher(Gtk.Application):
    ...
    def build_settings_menu(self):
        ...
        if platform.system() != "Darwin":
            settings_entries.extend(
                (
                    ("Test Mapped Controls", "Test Mapped Controls", 1),
                    ("Test Physical Inputs", "Test Physical Inputs", 1),
                )
            )
    ...
    def activate_settings_row(self, _listbox, row):
        ...
        if system == "Test Mapped Controls":
            ...
            self.show_setup(system, getattr(row, "control_player", 1))
    ...
    def launch_row(self, _listbox, row):
        ...
        if use_fceux:
            try:
                ...
                return
            command = emulator_command(game, console=self.current_system)
        else:
            ...
            command = emulator_command(
                game,
                console=self.current_system,
                append_configs=append_configs,
            )
        if not command:
            self.status.set_text(platform_help(console=self.current_system))
            return
        ...
        try:
            ...
            self.window.present()
            emulator = "FCEUX" if use_fceux else "RetroArch"
            self.status.set_text(f"Could not start {emulator}: {error}")
            return
